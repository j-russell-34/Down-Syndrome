...
cleandffull = cleandffull.rename({"de_gender":"Gender", "age_at_visit":"Age", "Left_DnSeg": "Left nbM Volume normalized to ICV", "Right_DnSeg": "Right nbM Volume normalized to ICV", "centiloid_value": "Amyloid (centiloids)", "tracer": "Tracer"}, axis=1)

ChBFlist = ('Left nbM Volume normalized to ICV', 'Right nbM Volume normalized to ICV')

for ChBF in ChBFlist:
    fig = sns.lmplot(data=cleandffull, x="Amyloid (centiloids)", y=ChBF)
    correlation = np.corrcoef(cleandffull['Amyloid (centiloids)'], cleandffull[ChBF])[0, 1]
    pearson = np.round(stats.pearsonr(cleandffull['Amyloid (centiloids)'], cleandffull[ChBF])[1], 10)
    if pearson == 0.0:
        plt.annotate(f"p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 5),
...
for ChBF in ChBFlist:
    ...
    correlation = np.corrcoef(cleandffull['Age'], cleandffull[ChBF])[0, 1]
    pearson = np.round(stats.pearsonr(cleandffull['Age'], cleandffull[ChBF])[1], 10)
    if pearson == 0.0:
        plt.annotate(f"p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 5),
                     xycoords='axes points', fontsize=8, color='red')
    else:
...
for ChBF in ChBFlist:
   ...
   correlation = np.corrcoef(males_df['Amyloid (centiloids)'], males_df[ChBF])[0, 1]
   pearson = np.round(stats.pearsonr(males_df['Amyloid (centiloids)'], males_df[ChBF])[1], 10)
   if pearson == 0.0:
       plt.annotate(f"Males: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 15),
                 xycoords='axes points', fontsize=8, color='red')
   else:
       plt.annotate(f"Males: p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
                 xy=(5, 15), xycoords='axes points', fontsize=8, color='red')
   correlation = np.corrcoef(females_df['Amyloid (centiloids)'], females_df[ChBF])[0, 1]
   pearson = np.round(stats.pearsonr(females_df['Amyloid (centiloids)'], females_df[ChBF])[1], 10)
   if pearson == 0.0:
       plt.annotate(f"Females: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 5),
                     xycoords='axes points', fontsize=8, color='red')
   else:
        plt.annotate(f"Females: p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
                     xy=(5, 5), xycoords='axes points', fontsize=8, color='red')
...
for ChBF in ChBFlist:
    ...
    correlation = np.corrcoef(males_df['Age'], males_df[ChBF])[0, 1]
    pearson = np.round(stats.pearsonr(males_df['Age'], males_df[ChBF])[1], 10)
    if pearson == 0.0:
        plt.annotate(f"Males: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 15),
                  xycoords='axes points', fontsize=8, color='red')
    else:
        plt.annotate(f"Males: p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
                  xy=(5, 15), xycoords='axes points', fontsize=8, color='red')
    correlation = np.corrcoef(females_df['Age'], females_df[ChBF])[0, 1]
    pearson = np.round(stats.pearsonr(females_df['Age'], females_df[ChBF])[1], 10)
    if pearson == 0.0:
        plt.annotate(f"Females: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 5),
                      xycoords='axes points', fontsize=8, color='red')
    else:
...
fig = sns.lmplot(data=cleandffull, x="Age", y="Amyloid (centiloids)", hue="Gender")
correlation = np.corrcoef(males_df['Age'], males_df["Amyloid (centiloids)"])[0, 1]
pearson = np.round(stats.pearsonr(males_df['Age'], males_df["Amyloid (centiloids)"])[1], 10)
if pearson == 0.0:
    plt.annotate(f"Males: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 15),
              xycoords='axes points', fontsize=8, color='red')
else:
    plt.annotate(f"Males: p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
              xy=(5, 15), xycoords='axes points', fontsize=8, color='red')
correlation = np.corrcoef(females_df['Age'], females_df["Amyloid (centiloids)"])[0, 1]
pearson = np.round(stats.pearsonr(females_df['Age'], females_df["Amyloid (centiloids)"])[1], 10)
if pearson == 0.0:
    plt.annotate(f"Females: p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 5),
                  xycoords='axes points', fontsize=8, color='red')
else:
     plt.annotate(f"Females: p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
                  xy=(5, 5), xycoords='axes points', fontsize=8, color='red')
plt.savefig(f"{study_cohort}, Age association with Amyloid Volume Separated by sex.png")
plt.clf()
    

fig = sns.lmplot(data=cleandffull, x="Age", y="Amyloid (centiloids)")
correlation = np.corrcoef(cleandffull['Age'], cleandffull["Amyloid (centiloids)"])[0, 1]
pearson = np.round(stats.pearsonr(cleandffull['Age'], cleandffull["Amyloid (centiloids)"])[1], 10)
if pearson == 0.0:
    plt.annotate(f"p < 0.0001 and r = {np.round(correlation, 5)}", xy=(5, 15),
              xycoords='axes points', fontsize=8, color='red')
else:
    plt.annotate(f"p = {np.round(pearson, 5)} and r = {np.round(correlation, 5)}",
              xy=(5, 15), xycoords='axes points', fontsize=8, color='red')
plt.savefig(f"{study_cohort}, Age association with Amyloid Volume.png")
plt.clf()
...
cleandffull.to_excel(f"ABC-DS DnSeg DF for R {x.year}{x.month}{x.day}.xlsx")
